...
const scoreByRules = (text, rules, fallback) => {
...
const updated = reasons.map((entry) => {
  ...
  const topic = scoreByRules(text, topicRules, "general");
  const tone = scoreByRules(text, toneRules, "neutral");
  const type = scoreByRules(text, typeRules, "general");
